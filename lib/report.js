...
export function buildReport ({ stats, meta, snapshot }) {
  ...
  if (meta.qc && meta.qc.grupos && meta.qc.grupos.length) {
    pdf.newPage()
    y = header('QC automático da segmentação')
    const rq = meta.qc.resumo || {}
    pdf.text(M, y, 'Escore = confiança (posterior máxima da rede) x coesão (maior componente conexo) x simetria (excesso E/D).', 8.5, { color: GRAY })
    y += 11
    pdf.text(M, y, 'Grupos e nomes são os do regressor de QC do SynthSeg 2.0; o escore é próprio e NÃO é o Dice predito daquele regressor.', 8.5, { color: GRAY })
    y += 11
    pdf.text(M, y, 'Corte de referência 0,65 (Billot et al., PNAS 2023) — calibre o seu na coorte.' +
      (rq.confiancaDisponivel === false ? ' Rede sem posteriores: confiança neutra nesta execução.' : ''), 8.5, { color: GRAY })
    y += 20
    y = sectionLabel(y, 'Por grupo tecidual')
    const cq = { name: M, bar: M + 232, esc: M + 352, cf: M + 400, co: M + 442, si: M + 484 }
    pdf.text(cq.name, y, 'GRUPO', 6.5, { bold: true, color: GRAY })
    pdf.text(cq.esc, y, 'ESCORE', 6.5, { bold: true, color: GRAY })
    pdf.text(cq.cf, y, 'CONF', 6.5, { bold: true, color: GRAY })
    pdf.text(cq.co, y, 'COES', 6.5, { bold: true, color: GRAY })
    pdf.text(cq.si, y, 'SIM', 6.5, { bold: true, color: GRAY })
    y += 5
    hairline(y)
    y += 13
    for (const q of meta.qc.grupos.filter(g => g.voxels > 0)) {
      const bad = q.alerta
      pdf.text(cq.name, y, q.pt.length > 44 ? q.pt.slice(0, 43) + '…' : q.pt, 8.5, bad ? { color: ACCENT } : {})
      // barra do escore
      const bw = 110
      pdf.setColor(...HAIR); pdf.rect(cq.bar, y - 6, bw, 5, true)
      pdf.setColor(...(bad ? ACCENT : [0.35, 0.72, 0.55]))
      pdf.rect(cq.bar, y - 6, Math.max(1, bw * Math.max(0, Math.min(1, q.escore))), 5, true)
      const n2 = (v) => v == null ? '—' : v.toFixed(2)
      pdf.text(cq.esc, y, n2(q.escore), 8.5, bad ? { bold: true, color: ACCENT } : { bold: true })
      pdf.text(cq.cf, y, n2(q.confianca), 8, { color: GRAY })
      pdf.text(cq.co, y, n2(q.coesao), 8, { color: GRAY })
      pdf.text(cq.si, y, n2(q.simetria), 8, { color: GRAY })
      y += 15
      if (y > pdf.pageH - 80) { pdf.newPage(); y = header('QC automático (cont.)') + 6 }
    }
    y += 6
    hairline(y)
    y += 14
    pdf.text(M, y, `Escore mínimo ${(rq.escoreMinimo ?? 0).toFixed(2)} · médio ${(rq.escoreMedio ?? 0).toFixed(2)}` +
      ((rq.gruposEmAlerta && rq.gruposEmAlerta.length)
        ? ` · em alerta: ${rq.gruposEmAlerta.join(', ')}`
        : ' · nenhum grupo abaixo do corte'), 9,
    (rq.gruposEmAlerta && rq.gruposEmAlerta.length) ? { bold: true, color: ACCENT } : { bold: true })
  }
  ...
  if (meta.surf && meta.surf.regioes && meta.surf.regioes.length) {
    ...
    pdf.text(M, y, 'Fluxo recon-all-clinical (navegador): SDFs white/pial (' + (meta.surf.motorSdf || 'SDF por EDT') + ') -> colocação pela energia', 8.5, { color: GRAY })
    y += 11
    pdf.text(M, y, 'do artigo (Gopinath 2025) -> espessura Fischl & Dale (teto 5 mm). Sem mris_fix_topology/sphere.reg: parcelas por amostragem DKT.', 8.5, { color: GRAY })
    y += 11
    const eu = meta.surf.euler
    pdf.text(M, y, 'QC de topologia (característica de Euler, esfera = 2): E = ' + (eu && eu.lh !== undefined ? eu.lh : '—') + ' · D = ' + (eu && eu.rh !== undefined ? eu.rh : '—') +
      (meta.surf.talairachRotulos ? ' · talairach.xfm por ' + meta.surf.talairachRotulos + ' COGs' : ''), 8.5, { color: GRAY })
    y += 18

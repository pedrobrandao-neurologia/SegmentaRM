...
/**
 * Inferência em blocos. `predictTile(f32, [d,h,w])` → Promise<{argmax: Uint8Array|Int32Array}>
 * O stitching usa o recorte central de cada bloco (metade da sobreposição de margem,
 * exceto nas bordas do volume).
 */
export async function tiledSegment ({ data, dims, tile = 96, overlap = 32, predictTile, onProgress }) {
  const [nx, ny, nz] = dims
  ...
  for (const tz of gz) {
    for (const ty of gy) {
      for (const tx of gx) {
        ...
        const { argmax } = await predictTile(block, [bw, bh, bd])
        ...
        for (let k = iz0; k < iz1; k++) {
          for (let j = iy0; j < iy1; j++) {
            for (let i = ix0; i < ix1; i++) {
              out[(tz.start + k) * nx * ny + (ty.start + j) * nx + (tx.start + i)] = argmax[k * bw * bh + j * bw + i]
            }
          }
        }
        done++

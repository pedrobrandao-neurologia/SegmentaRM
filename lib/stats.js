 ...
 */
export function computeStats (seg, img, dims, labelsMap, affine, voxVol = 1) {
  const [nx, ny, nz] = dims
  const nvox = nx * ny * nz
  const K = 256
  ...
  for (const key of Object.keys(labelsMap)) {
    ...
    if (idx === 0 || !count[idx]) {
      if (idx !== 0 && labelsMap[key]) {
        ...
        rows.push(makeRow(idx, labelsMap[key], 0, 0, 0, null))
      }
      continue
    }
    ...
    rows.push(makeRow(idx, name, n * voxVol, mean, sd, centroid))
  }
  ...
  for (const key of Object.keys(byBase)) {
    ...
    pairs.push({
      ...
      ptName: ptNameOf(p.E.name).replace(/ — esquerdo$/, ''),
      group: p.E.group,
      left: L,
      right: R,
      ai: 200 * (L - R) / (L + R)
...
function makeRow (index, name, volMm3, meanInt, sdInt, centroid) {
  return {
    index,
    name,
    ptName: ptNameOf(name),
    group: groupOf(name),

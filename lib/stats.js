...
export function statsToCSV (stats, meta, decimal = '.') {
  ...
  const L = []
  L.push(['exame', 'tipo', 'indice', 'rotulo', 'estrutura', 'grupo', 'hemisferio', 'volume_mm3', 'pct_encefalo', 'intensidade_media', 'intensidade_dp', 'centroide_x', 'centroide_y', 'centroide_z', 'espessura_mm', 'espessura_dp_mm', 'area_mm2'].join(sep))
  ...
  if (meta.surf && meta.surf.regioes) {
    for (const r of meta.surf.regioes) {
      L.push([sid, 'superficie', r.label, esc(r.name), esc(r.pt || r.base), 'Superfície cortical', r.hemi, fmt(r.volume_mm3), '', '', '', '', '', '', fmt(r.thickAvg, 2), fmt(r.thickStd, 2), fmt(r.area_mm2, 1)].join(sep))
    }
  }
  return L.join('\r\n')
}
...
export function statsToJSON (stats, meta) {
  return JSON.stringify({
    ...
    superficie: meta.surf ? {
      metodo: 'malhas white/pial por surface nets + Taubin sobre a parcelação DKT; espessura por pareamento branca-pial nos dois sentidos (Fischl & Dale 2000; aproximação, não é o mris_place_surface)',
